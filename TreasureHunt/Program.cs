...
class Program
{
    static bool _treasureFound = false;

    static void Main(string[] args)
    {
        var forest = new Forest(10, 20);
        forest.FoundTreasure += Forest_FoundTreasure;
        forest.ElementPositioning(new IElement[] { new Treasure() });
        forest.ElementPositioning(VehicleFactory.Build(6, forest));

        while (!_treasureFound)
        {
            Console.CursorVisible = false;
            forest.Print();

            var vehicles = forest.GetVehicles();
    ...
    static void Forest_FoundTreasure(object sender, EventArgs e)
    {
        _treasureFound = true;
        var vehicle = sender as Vehicle;
        Console.WriteLine($"\nThe treasure was found by the {vehicle?.Name} in the position {vehicle?.Coordinates}!");
...
//Forest forest = new(width: 30, height: 20);

//forest.Print();



//var sentido = Random.Shared.Next(0, 4);

//Direction direction = (Direction)sentido;
